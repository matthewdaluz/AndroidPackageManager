...
bool writePathHelperScripts(std::vector<std::string> &warnings) {
  ...
  helper << "#!/system/bin/sh\n";
  helper << "# Auto-generated by apmd (event-driven hotload)\n";
  helper << "APM_SANDBOX_ENV=" << shellQuote(apm::config::getSandboxPathEnvFile())
         << "\n";
  helper << "if [ -f \"$APM_SANDBOX_ENV\" ]; then\n";
  helper << "  . \"$APM_SANDBOX_ENV\"\n";
  helper << "fi\n";
  helper << "unset APM_SANDBOX_ENV\n";

  if (!writeAtomicFile(apm::config::getCommandsPathHelper(), helper.str(),
                       0755, true)) {
    warnings.push_back("Failed to write path helper script: " +
                       apm::config::getCommandsPathHelper());
    return false;
  }

  std::ostringstream exporter;
  exporter << "#!/system/bin/sh\n";
  exporter << "# Auto-generated by apmd (event-driven hotload)\n";
  exporter << "APM_PROFILE=" << shellQuote(apm::config::getGlobalProfileFile())
           << "\n";
  exporter << "if [ -f \"$APM_PROFILE\" ]; then\n";
  exporter << "  . \"$APM_PROFILE\" >/dev/null 2>&1\n";
  exporter << "fi\n";
  exporter << "hash -r 2>/dev/null || true\n";

  if (!writeAtomicFile(apm::config::getCommandsExportScript(), exporter.str(),
                       0755, true)) {
    warnings.push_back("Failed to write export script: " +
                       apm::config::getCommandsExportScript());
    return false;
  }
...
bool writeSandboxEnvFile(std::vector<std::string> &warnings) {
  std::ostringstream env;
  env << "#!/system/bin/sh\n";
  env << "# Auto-generated by apmd. Source this file to expose APM shims.\n";
  env << "APM_SHIM_DIR=" << shellQuote(apm::config::getApmBinDir()) << "\n";
  env << "case \":${PATH:-}:\" in\n";
  env << "  *:\"$APM_SHIM_DIR\":*) ;;\n";
  env << "  *)\n";
  env << "    if [ -z \"${PATH:-}\" ]; then\n";
  env << "      PATH=\"$APM_SHIM_DIR\"\n";
  env << "    else\n";
  env << "      PATH=\"$APM_SHIM_DIR:$PATH\"\n";
  env << "    fi\n";
  env << "    ;;\n";
  env << "esac\n";
  env << "export PATH\n";
  env << "unset APM_SHIM_DIR\n";

  if (!writeAtomicFile(apm::config::getSandboxPathEnvFile(), env.str(), 0644,
                       true)) {
    warnings.push_back("Failed to write sandbox env file: " +
                       apm::config::getSandboxPathEnvFile());
    return false;
  }
...
bool writeGlobalProfile(std::vector<std::string> &warnings) {
  std::ostringstream profile;
  profile << "# Auto-generated by apmd (event-driven hotload).\n";
  profile << "export ENV=" << shellQuote(apm::config::getGlobalProfileFile())
          << "\n";
  profile << "if [ -f " << shellQuote(apm::config::getSandboxPathEnvFile())
          << " ]; then\n";
  profile << "  . " << shellQuote(apm::config::getSandboxPathEnvFile())
          << "\n";
  profile << "fi\n";
  profile << "if [ -f " << shellQuote(apm::config::getCommandsPathHelper())
          << " ]; then\n";
  profile << "  . " << shellQuote(apm::config::getCommandsPathHelper())
          << "\n";
  profile << "fi\n";
  profile << "touch "
          << shellQuote(apm::config::getGlobalProfileSourcedMark())
          << " >/dev/null 2>&1 || true\n";

  if (!writeAtomicFile(apm::config::getGlobalProfileFile(), profile.str(),
                       0644, true)) {
    warnings.push_back("Failed to write global profile file: " +
                       apm::config::getGlobalProfileFile());
    return false;
  }

  ::chmod(apm::config::getGlobalProfileFile().c_str(), 0644);
  return true;
}
...
bool installShellHooks(std::vector<std::string> &warnings) {
  ...
  const std::string profileFile = apm::config::getGlobalProfileFile();
...
bool sourceProfileFile() {
  const std::string profile = apm::config::getGlobalProfileFile();
  if (!apm::fs::pathExists(profile)) {
    return false;
  }

  std::string cmd = ". " + shellQuote(profile);
  pid_t pid = ::fork();
  if (pid < 0) {
    return false;
  }
...
bool rebuild_command_index_and_shims(const std::string &triggerReason,
  ...
  if (!apm::fs::createDirs(apm::config::getApmRoot()) ||
    ...
    warnings.push_back("Failed to prepare sandbox/bin directories for hotload rebuild");
    ok = false;
  }
  ...
  if (!writeSandboxEnvFile(warnings)) {
    ok = false;
  }
  if (!writePathHelperScripts(warnings)) {
    ok = false;
  }
  if (!writeGlobalProfile(warnings)) {
    ok = false;
  }
...
void ensureProfileLoaded() {
  ...
  if (!apm::fs::pathExists(apm::config::getGlobalProfileFile())) {
    refreshPathEnvironment();
    return;
  }

  if (!sourceProfileFile()) {
    apm::logger::warn("export_path: failed to source global profile file");
  }
}
...
void generateEmulatorEnv() {
  ...
  out << "if [ -f " << shellQuote(apm::config::getSandboxPathEnvFile())
      << " ]; then\n";
  out << "  . " << shellQuote(apm::config::getSandboxPathEnvFile()) << "\n";
  out << "fi\n";
