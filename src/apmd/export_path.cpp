...
std::string buildTermuxEnvFileContent() {
  const std::string prefix = apm::config::getTermuxPrefix();
  const std::string home = apm::config::getTermuxHomeDir();
  const std::string tmp = apm::config::getTermuxTmpDir();

  std::ostringstream env;
  env << "# Generated by APM. Sourced by Termux package command shims.\n";
  env << "export PREFIX=" << shellQuote(prefix) << "\n";
  env << "export TERMUX_PREFIX=\"$PREFIX\"\n";
  env << "export HOME=" << shellQuote(home) << "\n";
  env << "export TERMUX_HOME=\"$HOME\"\n";
  env << "case \":${PATH:-}:\" in\n";
  env << "  *:\"$PREFIX/bin\":*) ;;\n";
  ...
  env << "export TMPDIR=" << shellQuote(tmp) << "\n";
  env << "export TERM=\"${TERM:-xterm-256color}\"\n";
  env << "export LANG=\"${LANG:-en_US.UTF-8}\"\n";
  env << "export SHELL=/system/bin/sh\n";
  env << "if [ -f \"$PREFIX/etc/tls/cert.pem\" ]; then\n";
  env << "  export SSL_CERT_FILE=\"$PREFIX/etc/tls/cert.pem\"\n";
  env << "  export GIT_SSL_CAINFO=\"$PREFIX/etc/tls/cert.pem\"\n";
  env << "  export CURL_CA_BUNDLE=\"$PREFIX/etc/tls/cert.pem\"\n";
  env << "  export REQUESTS_CA_BUNDLE=\"$PREFIX/etc/tls/cert.pem\"\n";
  env << "fi\n";
...
bool rebuild_command_index_and_shims(const std::string &triggerReason,
  ...
  std::string termuxEnvErr;
  if (!ensureTermuxEnvFile(&termuxEnvErr)) {
    warnings.push_back(termuxEnvErr.empty()
                           ? "Failed to write Termux runtime env file"
                           : termuxEnvErr);
    ok = false;
  }
...
bool ensureTermuxEnvFile(std::string *errorMsg) {
  bool ok = true;
  auto ensure = [&](const std::string &path) {
    if (!apm::fs::createDirs(path)) {
      ok = false;
    }
  };

  ensure(apm::config::getTermuxRoot());
  ensure(apm::config::getTermuxPrefix());
  ensure(apm::config::getTermuxHomeDir());
  ensure(apm::config::getTermuxTmpDir());

  const std::string content = buildTermuxEnvFileContent();
  const std::string path = apm::config::getTermuxEnvFile();

  std::string existing;
  if (ok && apm::fs::readFile(path, existing) && existing == content) {
    ::chmod(path.c_str(), 0644);
    if (errorMsg) {
      errorMsg->clear();
    }
  ...
  if (!ok || !writeAtomicFile(path, content, 0644, true)) {
    if (errorMsg) {
      *errorMsg = "Failed to write Termux runtime env file: " + path;
    }
    return false;
  }
